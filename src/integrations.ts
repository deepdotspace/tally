...
export const integrations: Record<string, { billing: 'developer' | 'user' }> = {
  google: { billing: 'user' },
  // Voice transcription (speech/speech-to-text, Whisper). The signed-in creator
  // pays for their own transcription, never the app owner. The /voice route is
  // auth-gated, so every caller has a JWT to forward + bill.
  speech: { billing: 'user' },
